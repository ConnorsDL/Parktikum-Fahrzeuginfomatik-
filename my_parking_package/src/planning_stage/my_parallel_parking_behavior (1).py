...
def show_examples(plan, map_w, spot_center_loc, spot_ori, front, left, right, back, parking_state_sequence):
    ...
    ## 2.compute state1_approaching points
        ...
        my_points = gen_curve_points_2d(                    # Generate points along curvce with:
            ...
            start_angle = np.pi+spot_ori-np.pi*15/180,      #   - starting angle of the curve
            curve_angle = np.pi*6/18)                       #   - angle to move starting angle along the circle                      
            
        
        # Visualize some computed state1_approaching points.
        num_points = my_points.shape[0]     # Points are in np.ndarray with each row being x,y values of a point.
        x = my_points[0,0]                  # 1st point x = [0,0] = [row,col]
        y = my_points[0,1]                  # 1st point y = [0,1] = [row,col]
        show_marker(10, x, y, g=1)
        ...
        # compute state2_approaching points
        ...
        # Visualize some computed state2_approaching points.
        ...
        # compute move_out_point
        my_move_out_point = gen_curve_points_2d(                                    # Generate points along curvce with:
            start       = my_parking_state2_point[num_points_parking_state2-1],     #   - start point on the circle
            radius      = 0.3,                                                      #   - radius of the cricle
            start_angle = spot_ori,                                                 #   - starting angle of the curve
            curve_angle = np.pi/2)                                                  #   - angle to move starting angle along the circle 
            
        
        # Visualize some computed move_out_point.
        num_move_out_point = my_move_out_point.shape[0]     # Points are in np.ndarray with each row being x,y values of a point.
        x = my_move_out_point[0,0]                          # 1st point x = [0,0] = [row,col]
        y = my_move_out_point[0,1]                          # 1st point y = [0,1] = [row,col]
        show_marker(20, x, y, g=1)
        x = my_move_out_point[num_move_out_point//4*1,0]
        y = my_move_out_point[num_move_out_point//4*1,1]
        show_marker(21, x, y, g=1)
        x = my_move_out_point[num_move_out_point//4*2,0]
        y = my_move_out_point[num_move_out_point//4*2,1]
        show_marker(22, x, y, g=1)
        x = my_move_out_point[num_move_out_point//4*3,0]
        y = my_move_out_point[num_move_out_point//4*3,1]
        show_marker(23, x, y, g=1)
        x = my_move_out_point[num_move_out_point-1,0]
        y = my_move_out_point[num_move_out_point-1,1]
        show_marker(24, x, y, g=1)
        

    
    ## 3.add an approaching state.
        my_approaching_state = get_approaching_state(map_w, my_way_point[0], plan)  # Get approaching state.
    ...
    ## 5.waiting_state 
        my_waiting_state2 = PredefinedWaitingState(                  # Create state with:
            ...
            time_limit = config.circuit_mode.parking_stop_time,      #   - time to wait
            point = my_way_point[len(my_way_point)-1],               #   - position to wait at
            tangent    = vector_of(spot_ori),                        #   - orientation to wait at
    ...
    ## 6.move_away_state
        ...
        my_move_out_state = PredefinedDrivingState(
            state_name = "my_move_out_state",
            state_id   = "my_move_out_state")
        my_move_out_state.set_points(                                                          # Waypoints are added like they would be driven forward.
            my_move_out_point, 
            np.linspace( 0.1,0.02, len(my_move_out_point)),
            np.linspace(0.1, 0.02 , len(my_move_out_point)))
        my_move_out_state.target_speed_kmh = config.circuit_mode.parking_max_speed_mps * 36.0  # Set negative driving speed.
        my_move_out_state.indicator_light.turn_indicator_left = True
        parking_state_sequence.states.append(my_move_out_state)
