...
//1: Iterate over visible street in current camera image. 
    ...
    //3: Get pixel in image of world points.
        ...
        // ORIGINAL IMAGE.
        cv::Mat original_img = input.ipm_raw_gray;
        cv::imshow("Show origal image in opencv",original_img);
        cv::waitKey(1);

        std::pair<float,bool>result = find_closest_white_pixel_transverse(
            original_img,
            glm::vec2(0,0),
            glm::vec2(1,1),
            1,1,
        ...
        // std::cout << std::pair<float,bool>result << std::endl ;
